<template>
  <section class="flex w-full flex-col gap-6">
    ...
    <QuickProductEditorDialog v-model:open="quickEditOpen" :categories="catalog.categories" :product="detail" :saving="saving" @save="saveProduct" />
...
import QuickProductEditorDialog from "./QuickProductEditorDialog.vue";
import { productDetailToForm, type ProductForm } from "./product-form";
...
import { onDeleteProduct, onGetCatalogAdminData, onGetProductAdminDetail, onSaveProduct, onSetProductStatus } from "@/server/catalog/admin.telefunc";
...
async function openEdit(id: number) {
  detail.value = null;
  try {
    const result = await runTelefunc(() => onGetProductAdminDetail({ id }), { notifyError: false });
    detail.value = productDetailToForm(result.product);
    quickEditOpen.value = true;
  } catch (cause) {
    quickEditOpen.value = false;
    error.value = userErrorMessage(cause, "无法读取商品信息。");
  }
}
...
async function saveProduct(form: ReturnType<typeof import("./product-form").formToSaveInput>) { saving.value = true; try { await runTelefunc(() => onSaveProduct(form), { successMessage: form.id ? "商品已保存。" : form.status === "ACTIVE" ? "商品已创建并上架。" : "商品已创建。" }); dialogOpen.value = false; quickEditOpen.value = false; detail.value = null; await loadCatalog(); } finally { saving.value = false; } }
